...
namespace Microsoft.Teams.Apps.EmployeeTraining.Authentication
{
    ...
    public class MustBeLnDTeamMemberHandler : AuthorizationHandler<MustBeLnDTeamMemberRequirement>
    {
            ...
            MustBeLnDTeamMemberRequirement requirement)
        {
            ...
            {
                this.telemetryClient.TrackTrace($"Valid Request with queryString:{httpContext.Request.QueryString.Value}");
        ...
        {
            this.telemetryClient.TrackTrace($"MustbeLndTeam:ValidateUserIsPartOfTeamAsync with teamid:{teamId} and UserAAD:{userAadObjectId}");
            ...
            {
                this.telemetryClient.TrackTrace($"MustbeLndTeam:No Cached Entry with teamid:{teamId} and UserAAD:{userAadObjectId}");
                ...
                try
                {
                    ...
                    {
                        this.telemetryClient.TrackTrace($"MustbeLndTeam:In-valid Member teamid:{teamId} and UserAAD:{userAadObjectId}");
                        return isUserValidMember;
                    }
                }
                catch(Exception ex)
                {
                    this.telemetryClient.TrackException(new Exception($"MustbeLndTeam:ValidateUserIsPartOfTeamAsync Failed with exception {ex.Message}"));
                }
            }
